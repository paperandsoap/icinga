      ...
      module Language
        ...
        def search(obj, query=nil, sort=nil, start=0, rows=1000, &block)
          ...
          case obj
            ...
            when :environment
              search_environments(start, rows, &block)
            else
        ...
        def search_roles(start, rows, &block)
          Dir.glob(File.join(Chef::Config[:data_bag_path], "role", "*.json")).map do |f|
            # parse and hashify the node
        ...
        def search_environments(start, rows, &block)
          Dir.glob(File.join(Chef::Config[:data_bag_path], "environment", "*.json")).map do |f|
            # parse and hashify the node
            environment = JSON.parse(IO.read(f))
            if @_query.match(environment.to_hash)
              @_result << environment
            end
          end
        end
